...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        '''
        # total_samples = self.calculate_samples()

        # new_parameters = copy.deepcopy(self.model.state_dict())
        # for key in new_parameters.keys():
        #     new_parameters[key] = torch.zeros_like(new_parameters[key], dtype=torch.float32)

        # mu = 0.05  # Regularization term for FedProx
        # for user in self.selected_users:
        #     user_parameters = user.model.state_dict()
        #     cur_weight = user.train_samples / total_samples

        #     for key in new_parameters.keys():
        #         new_parameters[key] += cur_weight * (user_parameters[key] + mu * (new_parameters[key] - user_parameters[key]))
